...
namespace DogCheck
{
    ...
    public class SoftKey
    {
        ...
        private int ReadStringEx(int addr, ref string outstring, string DevicePath)
        {
            ...
            byte[] buf = new byte[1];
            //先从地址0读到以前写入的字符串的长度
            ret = YReadEx(buf, (short)addr, (short)1, "FFFFFFFF", "FFFFFFFF", DevicePath);
            if (ret != 0) return ret;
            nlen = buf[0];
            //再读取相应长度的字符串
            return YReadString(ref outstring, addr + 1, nlen, "FFFFFFFF", "FFFFFFFF", DevicePath);
        ...
        public int CheckKeyByReadEprom()
        {
            ...
            for (n = 0; n < 255; n++)
            {
                ...
                if ((ret == 0) && (outstring.CompareTo("ZhangJin") == 0)) return 0;
            }
            return -92;
        }
        ...
        private int Sub_CheckKeyByEncstring(string InString, string DevicePath)
        {
            ...
            // 通过自定义算法获得加密后的字符串
            // 传递的参数为 密钥【32个16进制数】
            outstring_2 = StrEnc(InString, "7629637A551D5C04F401954287E5DEDD");
